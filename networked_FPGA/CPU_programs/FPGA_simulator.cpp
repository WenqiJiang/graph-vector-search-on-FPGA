...
{ 
    ...
    std::cout << "Usage: " << argv[0] << " <Tx (CPU) IP_addr> <Tx send_port> <Rx recv_port> <nprobe>" 
	...
	int D = 128;
	if (argc >= 7) {
		D = strtol(argv[6], NULL, 10);
	}

    size_t query_num = 10000;
    if (argc >= 8) {
        query_num = strtol(argv[7], NULL, 10);
    }
    ...
    std::thread t_send(thread_F2C,
		IP_addr, send_port, query_num, &start_send, &finish_recv_query_id, D, TOPK);
    std::thread t_recv(thread_C2F,
		recv_port, query_num, &start_send, &finish_recv_query_id, D, TOPK);
...
void thread_F2C(
	...
	// unsigned int send_port = send_thread_input.send_port;
	...
	size_t bytes_output_per_query = bytes_results_vec_ID + bytes_results_dist;
	...
	int sock = send_open_conn(IP_addr, send_port);
...
void thread_C2F(
    ...
    // unsigned int recv_port = recv_thread_input.recv_port;
	...
	int sock = recv_accept_conn(recv_port);
	...
	size_t bytes_input_per_query = bytes_header + bytes_vec;
