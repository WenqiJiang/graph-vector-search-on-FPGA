...
#pragma HLS stream variable=s_distances_upper_levels depth=512
	...
	hls::stream<int> s_num_neighbors_upper_levels_replicated[rep_factor_s_num_valid_candidates_burst];
#pragma HLS stream variable=s_num_neighbors_upper_levels_replicated depth=16
	...
	task_scheduler(
		...
		// in streams
		s_num_neighbors_upper_levels_replicated[0],
...
#pragma HLS stream variable=s_valid_candidates_replicated depth=512
	...
	hls::stream<int> s_finish_query_replicate_num_candidates; // finish all queries
#pragma HLS stream variable=s_finish_query_replicate_num_candidates depth=16
...
#pragma HLS stream variable=s_finish_query_replicate_candidates depth=16
	...
	replicate_s_control<rep_factor_s_num_valid_candidates_burst, int>(
		// in (initialization)
		query_num,
		// in (stream)
		s_num_neighbors_upper_levels,
		...
		// out (stream)
		s_num_neighbors_upper_levels_replicated,
		s_finish_query_replicate_num_candidates
	);
	...
	replicate_s_read_iter_and_s_data<rep_factor_s_num_valid_candidates_burst, cand_t>(
		...
		query_num,
		// in (stream)
		s_num_neighbors_upper_levels_replicated[1],
		s_fetched_neighbor_ids,
		s_finish_query_replicate_num_candidates,
