...
void results_collection(
	...
	while (true) {
		...
		for (int qid = 0; qid < query_num; qid++, cnt_total_qid++) {

			debug_out_per_query_dists[cnt_debug_out_per_query_dists_ids] = -1;
					...
					for (int bid = 0; bid < cand_batch_size; bid++) {
						...
						// insert new values but without sorting
							...
							#pragma HLS pipeline II=1
								...
								// write debug signals
								debug_out_per_query_dists[cnt_debug_out_per_query_dists_ids] = reg.dist;
								debug_out_per_query_ids[cnt_debug_out_per_query_dists_ids] = reg.node_id;
								cnt_debug_out_per_query_dists_ids++;
