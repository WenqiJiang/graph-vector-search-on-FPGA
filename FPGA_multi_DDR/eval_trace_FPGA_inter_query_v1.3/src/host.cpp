...
int main(int argc, char** argv)
{
    ...
    if (query_batch_size > query_num) {
		query_batch_size = query_num;
		std::cout << "query_batch_size is larger than query_num, set to query_num=" << query_num << std::endl;
	}
...
// #endif
...
#if N_CHANNEL >= 2
	OCL_CHECK(err, err = krnl_vector_add.setArg(arg_counter++, buffer_debug_out_per_query_dists_chan_1));
#endif
#if N_CHANNEL >= 3
	OCL_CHECK(err, err = krnl_vector_add.setArg(arg_counter++, buffer_debug_out_per_query_dists_chan_2));
#endif
#if N_CHANNEL >= 4
	OCL_CHECK(err, err = krnl_vector_add.setArg(arg_counter++, buffer_debug_out_per_query_dists_chan_3));
#endif

	OCL_CHECK(err, err = krnl_vector_add.setArg(arg_counter++, buffer_debug_out_per_query_ids_chan_0));
#if N_CHANNEL >= 2
	OCL_CHECK(err, err = krnl_vector_add.setArg(arg_counter++, buffer_debug_out_per_query_ids_chan_1));
#endif
#if N_CHANNEL >= 3
	OCL_CHECK(err, err = krnl_vector_add.setArg(arg_counter++, buffer_debug_out_per_query_ids_chan_2));
#endif
#if N_CHANNEL >= 4
	OCL_CHECK(err, err = krnl_vector_add.setArg(arg_counter++, buffer_debug_out_per_query_ids_chan_3));
#endif
	...
	// Put the results into output file
	...
	assert (N_CHANNEL == 1); // only support 1 channel debug serialization now
	// detect the number of elements per debug array, and only write those
	int cnt_debug_num_elements_per_array = 0;
	for (int i = 0; i < query_num_after_offset; i++) {
		assert (debug_out_per_query_dists_chan_0[cnt_debug_num_elements_per_array] == -1);
		assert (debug_out_per_query_ids_chan_0[cnt_debug_num_elements_per_array] == -1);
		cnt_debug_num_elements_per_array++;
		
		while (true) {
			if (debug_out_per_query_dists_chan_0[cnt_debug_num_elements_per_array] == -2
				&& debug_out_per_query_ids_chan_0[cnt_debug_num_elements_per_array] == -2) {
				cnt_debug_num_elements_per_array++;
				break;
			} else if (debug_out_per_query_dists_chan_0[cnt_debug_num_elements_per_array] != -2
				&& debug_out_per_query_ids_chan_0[cnt_debug_num_elements_per_array] != -2) {
				cnt_debug_num_elements_per_array ++;
			} else {
				std::cout << "Error: debug signal format is wrong" << std::endl;
				std::cout << "cnt_debug_num_elements_per_array=" << cnt_debug_num_elements_per_array << std::endl;
				std::cout << "dist=" << debug_out_per_query_dists_chan_0[cnt_debug_num_elements_per_array] << std::endl;
				std::cout << "id=" << debug_out_per_query_ids_chan_0[cnt_debug_num_elements_per_array] << std::endl;
				exit(1);
			}
		}
	}
