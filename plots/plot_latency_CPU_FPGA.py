...
def plot_latency(dataset='SIFT1M', graph_type="HNSW", max_degree=64, ef=64, batch_sizes=[1,2,4,8,16],
                 show_FPGA=True, show_CPU=True, add_CPU_network_latency=True):
    ...
    #     title += f', Recall@1≥{recall_1_CPU*100:.2f}%'
    if recall_10_CPU is not None:
        title += f', Recall@10≥{recall_10_CPU*100:.2f}%'
    ...
    plt.savefig(f'./images/latency_CPU_FPGA_{dataset}_{graph_type}.png', transparent=False, dpi=200, bbox_inches="tight")
...
if __name__ == "__main__":
    ...
    for dataset in datasets:
        for graph_type in graph_types:
            plot_latency(dataset, graph_type, show_FPGA=True, show_CPU=True, add_CPU_network_latency=True)