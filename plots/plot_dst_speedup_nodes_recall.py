...
Example usage:
	...
	python plot_dst_speedup_nodes_recall.py --graph_type HNSW --dataset SIFT1M --max_degree 64 --ef 64 --df_path ../perf_test_scripts/saved_df/throughput_FPGA_inter_query_4_chan.pickle

	# plot all (hard-coded) settings
	python plot_dst_speedup_nodes_recall.py --plot_all 1 --df_path ../perf_test_scripts/saved_df/throughput_FPGA_inter_query_4_chan.pickle
"""
...
def plot_heatmap(df, graph_type, dataset, max_degree, ef):

	# select rows 
	df = df.loc[(df['graph_type'] == graph_type) & (df['dataset'] == dataset) & (df['max_degree'] == max_degree) & (df['ef'] == ef)]

	# get the speedup, nodes, recall
	max_mg = 6 # max number of groups in the pipeline
	max_mc = 4 # max candidates per group
	...
	fig, ax_array = plt.subplots(1, 3, figsize=(16, 1.5))
	...
	plt.savefig('./images/dst_speedup_nodes_recall_{}_{}_MD{}_ef{}.png'.format(dataset, graph_type, max_degree, ef), transparent=False, dpi=200, bbox_inches="tight")
		...
		for graph_type in graph_types:
			for dataset in datasets:
				for max_degree in max_degrees:
					for ef in efs:
						plot_heatmap(df, graph_type, dataset, max_degree, ef)
	else:
		plot_heatmap(df, args.graph_type, args.dataset, args.max_degree, args.ef)
