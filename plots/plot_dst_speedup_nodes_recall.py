...
def plot_heatmap(df, graph_type, dataset, max_degree, ef, suffix='inter_query', max_mg=6, max_mc=3):
	...
	def plot_all():
		...
		if suffix == "inter_query":
			plt.suptitle(f'(a) Inter-query parallelism, {graph_type} on {dataset}, max_degree={max_degree}', fontsize=14, y=1.2)
		...
		plt.savefig('./images/dst_speedup_nodes_recall/dst_speedup_nodes_recall_{}_{}_MD{}_ef{}_{}.png'.format(dataset, graph_type, max_degree, ef, suffix), transparent=False, dpi=200, bbox_inches="tight")
	...
	def plot_each_subplot(mode='speedup'):
		...
		if mode == "speedup":
			if suffix == "inter_query":
				ax.set_title('Speedup over BFS (Inter-query)', fontsize=label_font)
			elif suffix == "intra_query":
				ax.set_title('Speedup over BFS (Intra-query)', fontsize=label_font)
			plt.savefig('./images/dst_speedup_nodes_recall/dst_speedup_{}_{}_MD{}_ef{}_{}.png'.format(dataset, graph_type, max_degree, ef, suffix), transparent=False, dpi=200, bbox_inches="tight")
		elif mode == "nodes":
			ax.set_title('Normalized average #hops per search', fontsize=label_font)
			plt.savefig('./images/dst_speedup_nodes_recall/dst_nodes_{}_{}_MD{}_ef{}_{}.png'.format(dataset, graph_type, max_degree, ef, suffix), transparent=False, dpi=200, bbox_inches="tight")
		elif mode == "recall":
			ax.set_title('Recall R@10', fontsize=label_font)
			ax.text(0.75, 1.12, f"{dataset}, {graph_type}", horizontalalignment='left', verticalalignment='center', transform=ax.transAxes, fontsize=legend_font)
			plt.savefig('./images/dst_speedup_nodes_recall/dst_recall_{}_{}_MD{}_ef{}_{}.png'.format(dataset, graph_type, max_degree, ef, suffix), transparent=False, dpi=200, bbox_inches="tight")
